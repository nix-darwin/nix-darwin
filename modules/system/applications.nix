...

{
  config = {
    ...
    system.build.applications = pkgs.buildEnv {
      ...
      ++ config.environment.extraAppPaths;
    };
    ...
    system.activationScripts.applications.text = ''
      	  ...
      	  for path in ${lib.concatStringsSep " " config.environment.extraAppPaths}; do
      		${lib.getExe pkgs.rsync} "''${rsyncFlags[@]}" "${config.system.build.applications}/$path/" "$path"
      	  done

      	  echo "done setting up /Applications/Nix Apps." >&2
