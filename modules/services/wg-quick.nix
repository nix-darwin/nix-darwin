...
let
    ...
    { ... }:
    {
      options = {
        ...
        presharedKeyFile = mkOption {
          type = types.nullOr types.str;
          default = null;
          description = "Optional, path to file containing the pre-shared key for this peer.";
        };
    ...
    { ... }:
    {
      options = {
        ...
        mtu = mkOption {
          type = types.nullOr types.int;
          default = null;
          description = "MTU to set for this interface, automatically set if not specified";
        };
  ...
  generateInterfaceScript = name: text: ((pkgs.writeShellScriptBin name text) + "/bin/${name}");

  generatePostUpPSKText =
    name: interfaceOpt:
    map (
      peer:
  ...
  generateInterfacePostUp =
    name: interfaceOpt: generateInterfaceScript "postUp.sh" (generatePostUpText name interfaceOpt);
  ...
  generateInterfaceConfig =
    ...
    ''
    + optionalString (interfaceOpt.privateKeyFile != null || interfaceOpt.postUp != "") ''
...
in
{
  ...
  config = mkIf (cfg.interfaces != { }) {
    launchd.daemons = mapAttrs' generateLaunchDaemonAttrs (
      filterAttrs (name: interfaceOpt: interfaceOpt.autostart) config.networking.wg-quick.interfaces
    );

    environment.etc = mapAttrs' generateInterfaceAttrs config.networking.wg-quick.interfaces;
