...
let
  ...
  sshConf = settingsFormat.generate "sshd.conf-settings" (
...
in
{
          ...
          { name, ... }:
          {
            ...
            options = {
              ...
              Include = lib.mkOption {
                type = lib.types.nullOr (lib.types.listOf lib.types.str);
                default = [ "/etc/ssh/sshd_config.d/*" ]; # macOS default
                description = ''
  ...
  config = {
    ...
    environment.etc."ssh/sshd_config".source = sshConf;

    system.checks.text = lib.mkAfter ''
      sshd -G -T -f ${sshConf} > /dev/null
    '';
  };
}
