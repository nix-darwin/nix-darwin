...
let
  ...
  hasWork = cfg.upgrade || cfg.packages != { } || cfg.cleanup || homebrewIds != [ ];
      ...
      ''
        ...
        if (( listStatus != 0 )); then
          ...
          if echo "''${listOutput}" | grep -qi "not signed in"; then
            echo >&2 "login required; skipping App Store installs/upgrades/cleanup"
            exit 0
          fi
        fi
        ...
        ${optionalString cfg.upgrade ''
          runAsUser ${getExe cfg.package} upgrade || true
        ''}
  ...
  options.programs.mas = {
    ...
    user = mkOption {
      ...
      description = ''
        ...
        for installs or upgrades to succeed.
      '';
    };
    ...
    upgrade = mkOption {
      ...
      default = true;
      description = ''
        Whether to run {command}`mas upgrade` during system activation in addition to installing the
        configured apps.
      '';
    };
    ...
    cleanup = mkOption {
      ...
      description = ''
        ...
        This runs before install/upgrade; any app id not in either set will be removed.
      '';
    };
  };
