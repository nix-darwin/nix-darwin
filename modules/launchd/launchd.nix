        ...
        if (builtins.isNull val) then
          ...
          (map (
            ...
            lib.warnIf (lib.hasInfix "&amp;" item) ''
              A value for `ProgramArguments` contains the literal string `&amp;`. This is no longer necessary and
              will lead to double-escaping, as nix-darwin now automatically escapes special characters.
            '' item
          ) val);
    };
