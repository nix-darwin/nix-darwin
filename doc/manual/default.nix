...
let
  ...
  gitHubDeclaration =
    ...
    let
      urlRef = if ref != null then ref else "master";
    in
    {
        ...
        declarations = map (
          ...
          if lib.hasPrefix (toString prefix) (toString decl) then
            gitHubDeclaration "nix-darwin" "nix-darwin" revision (
              lib.removePrefix "/" (lib.removePrefix (toString prefix) (toString decl))
            )
...
rec {
  ...
  optionsJSON =
    runCommand "options.json" { meta.description = "List of nix-darwin options in JSON format"; }
      ''
